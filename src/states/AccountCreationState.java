...
public class AccountCreationState extends State implements ActionListener
{
    ...
    public void onInitialize()
    {
        ...
        username = new TextField(leftPos, topPos, TEXT_WIDTH, TEXT_HEIGHT, "Enter Username");
        add(username);
        email = new TextField(rightPos, topPos, TEXT_WIDTH, TEXT_HEIGHT, "Enter Email");
        add(email);
        password = new PasswordArea(leftPos, midPos, TEXT_WIDTH, TEXT_HEIGHT, "Enter Password");
        add(password);
    ...
    public void actionPerformed(ActionEvent e)
    {
        if(e.getSource().equals(toLogin)){
            ...
            if(password.getPassword().equals(verifyPassword.getPassword())){
                
            } else {
                FixedWidthMessage.showMessage("Please make sure your passwords match.", "Verify password", JOptionPane.INFORMATION_MESSAGE, getWidth()/2);
            }
        }
    }
